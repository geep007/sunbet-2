...
document.addEventListener("DOMContentLoaded", function () {
  // Get the container specifically under "Manage Bank Accounts & Cards"
  const container = document.querySelector(
    'div[sf-name="manage-bank-account"]'
  );
  ...
  function setupDeleteHandlers() {
    ...
    const deleteButtons = document.querySelectorAll(
      ".manage-card-existing-details-wrapper .buttons[data-entity-id]"
    );
    ...
    deleteButtons.forEach((deleteButton) => {
      deleteButton.addEventListener("click", function (e) {
        ...
        const entityId = this.getAttribute("data-entity-id"); // Get the entity ID
        if (entityId) {
          requestDeleteOTP(entityId, "email"); // Trigger the delete OTP process
        }
      });
    });
  }

  // // Function to set up the "Add Account" button handler
  // function setupAddAccountHandler() {
  //   const addAccountButton = document.querySelector(
  //     'a[sf-to="add-bank-account"]'
  //   );
  //   if (addAccountButton) {
  //     addAccountButton.addEventListener("click", function (e) {
  //       e.preventDefault(); // Prevent default link behavior
  //       showAddBankAccountSection(); // Show the "Add Bank Account" section
  //     });
  //   }
  // }

  // // Function to show the "Add Bank Account" section
  // function showAddBankAccountSection() {
  //   const manageSection = document.querySelector(
  //     'div[sf-name="manage-bank-account"]'
  //   );
  //   const addSection = document.querySelector(
  //     'div[sf-name="add-bank-account"]'
  //   );

  //   if (manageSection && addSection) {
  //     manageSection.style.display = "none"; // Hide the manage section
  //     addSection.style.display = "block"; // Show the add section
  //   } else {
  //     console.error("Manage or Add Bank Account section not found.");
  //   }
  // }
  ...
  function createBankAccountCard(entity) {
    ...
    return `
      <div class="manage-card-existing-details-wrapper">
        <div class="h3-heading-styling is-wallet-heading">${entity.accountHolder}</div>
        <div class="text-size-small">${bankName}</div>
        <div class="text-size-small">${accountNumber}</div>
        <button type="button" class="buttons is-regular is-not-caps" data-entity-id="${entity.id}">
          <img src="https://cdn.prod.website-files.com/66956eb2aafafe3229e15ef3/67e0b1a08e4343c8eb3b7309_delete_24dp_5F6368_FILL0_wght400_GRAD0_opsz24%20(2).svg" loading="lazy" alt="" class="tag-svg">
          <div class="is-caps">DELETE ACCOUNT</div>
        </button>
      </div>
    `;
  }
  ...
  function loadPaymentEntities() {
    simlBC.getPaymentEntities(function (err, data) {
      ...
      if (data && data.paymentEntities) {
        // Find the container for the payment entities
        const paymentEntitiesContainer =
          document.querySelector(".div-block-47");
        if (!paymentEntitiesContainer) {
          console.error("Payment entities container not found.");
          return;
        }

        // Clear existing payment entities
        paymentEntitiesContainer.innerHTML = "";
        ...
        verifiedEntities.forEach((entity) => {
          const cardHtml = createBankAccountCard(entity);
          paymentEntitiesContainer.insertAdjacentHTML("beforeend", cardHtml);
        });
