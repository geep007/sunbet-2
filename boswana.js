...
async function handleFormSubmit(event) {
  ...
  try {
    ...
    // Validate age
    const dateOfBirth = getFormattedDateOfBirth(); // Ensure this function returns the DOB in "YYYY-MM-DD" format
    ...
    const formData = {
      ...
      idNumberType: idType === "Passport" ? "PSP" : "DNI",
      ...
      currencyCode: "BWP",
      marketing: {
        optInEmail: document.getElementById("Allow-Marketing-Mails-2")?.checked || false,
      },
    };
    ...
    showError(form, error.message); // Display the error message to the user
  } finally {
    if (submitButton) {
      submitButton.value = "Create Account";
      submitButton.disabled = false;
  ...
  if (tabsContainer) {
    ...
    const observer = new MutationObserver(() => {
      ...
      if (currentTab >= 2) {
        ...
        const surnameInput = document.querySelector("#Surname-2");
        ...
        if (firstNameInput) {
          ...
          console.log("Stored First Name:", firstNameInput.value);
        }
        if (surnameInput) {
          localStorage.setItem("surname", surnameInput.value);
          console.log("Stored Surname:", surnameInput.value);
        }
      }
...
case "Mobile-Number-2":
  if (value === "") {
    ...
    const validPrefixes = ["71", "72", "73", "74", "75", "76", "77", "78", "79"];
    let prefix = "";

    if (digits.length === 11 && digits.startsWith("267")) {
      prefix = digits.substring(3, 5);
      isValid = validPrefixes.includes(prefix);
      errorMessage = isValid
        ? ""
        : "Invalid Botswana mobile prefix after 267";
    } else if (digits.length === 8) {
      prefix = digits.substring(0, 2);
      isValid = validPrefixes.includes(prefix);
      if (isValid) {
        // Prepend 267 and update the input value
        digits = "267" + digits;
        field.value = digits;
        errorMessage = "";
      } else {
        errorMessage = "Invalid Botswana mobile prefix";
      }
    } else {
      isValid = false;
      errorMessage =
        digits.length < 8
          ? "Please enter a valid Botswana mobile number (8 digits or 267 + 8 digits)"
  ...
  break;
    ...
    case "ID-2":
      if (value === "") {
        ...
        const saIDValid = /^\d{9}$/.test(value);
        const passportValid = /^[A-Za-z][A-Za-z0-9]{5,13}$/.test(value);
        isValid = saIDValid || passportValid;
        errorMessage = isValid ? "" : "Enter valid ID (9 digits) or passport";
      }
      break;
...
function getFormattedDateOfBirth() {
  const day = document.getElementById("Day-2")?.value.trim().padStart(2, "0") || "";
  const month = document.getElementById("Month-2")?.value.trim().padStart(2, "0") || "";
  const year = document.getElementById("Year-2")?.value.trim() || "";

  if (!day || !month || !year) {
    throw new Error("Please enter day, month, and year");
  }
