document.addEventListener("DOMContentLoaded", () => {
  ...
  function fetchAndDisplayLimits() {
    simlBC.getLimits((err, data) => {
      if (err) {
        console.error("Error fetching limits:", err);
        return;
      }

      const depositLimitInput = document.getElementById("field-5");

      if (data.dailyDeposit && data.dailyDeposit.pendingLimit) {
        depositLimitInput.value = data.dailyDeposit.pendingLimit;
        depositLimitInput.style.backgroundColor = "#1B2838";
        depositLimitInput.style.color = "#F5A623";
        depositLimitInput.style.opacity = "1";
        depositLimitInput.style.border = "1px solid #F5A623";
        depositLimitInput.disabled = false;
      } else if (data.dailyDeposit && data.dailyDeposit.activeLimit) {
        depositLimitInput.value = data.dailyDeposit.activeLimit;
      }
    });
  }

  function updateDepositLimit(newLimit) {
    const limitData = {
      limitTypes: ["dailyDeposit"], // Only sending the limit type
    };

    console.log("Sending limit data:", limitData); // Debug log

    simlBC.setPendingLimits(limitData, function (err, data) {
      // Create a function to display messages (error or success)
      function displayMessage(message, isError = false) {
        ...
        if (isError) {
          messageElement.style.backgroundColor = "#FF464F20";
          messageElement.style.color = "#FF464F";
          messageElement.style.border = "1px solid #FF464F";
        } else {
          messageElement.style.backgroundColor = "#4CAF5020";
        ...
        // Find the appropriate place to insert the message
        const buttonWrapper = document.querySelector(
          ".daily-deposit-button-wrapper"
        );
        if (buttonWrapper) {
      ...
      if (err) {
        console.error("Failed to update deposit limit:", err);

        // Extract error message from the API response
        let errorMessage = "Failed to update deposit limit";

        if (err.errors && err.errors.length > 0 && err.errors[0].detail) {
          errorMessage = err.errors[0].detail;
        }
      ...
      // Now we need to set the actual limit value using setLimits
      const limitValueData = {
        dailyDepositLimit: parseInt(newLimit),
      };

      simlBC.setLimits(limitValueData, function (err, data) {
        if (err) {
          console.error("Failed to set limit value:", err);

          // Extract error message from the API response
          let errorMessage = "Failed to set limit value";
        ...
        console.log("Limit value set successfully:", data);

        // Display success message
        let successMessage = "Deposit limit updated successfully";
        ...
        // Refresh the display to show new pending limit
  ...
  const authorizeButton = Array.from(
    document.querySelectorAll(".w-button")
  ).find(
    (button) =>
      button.textContent.trim().toLowerCase() === "update deposit limit"
  );
  ...
  if (authorizeButton) {
    authorizeButton.addEventListener("click", (e) => {
      ...
      const newLimit = document.getElementById("field-5").value;

      if (!newLimit) {
        alert("Please enter a deposit limit");
        return;
      }

      // First request the PIN
      simlBC.requestSessionPin(
        ...
        function (requestErr, requestData) {
          ...
          const otpHandler = function (event) {
            ...
            simlBC.verifySessionPin(
              ...
              function (verifyErr, verifyData) {
                ...
                // After successful verification, update the deposit limit
                setTimeout(() => {
                  updateDepositLimit(newLimit);
                  // Display success message after limit update
                  const successMessage = document.createElement("div");
                  successMessage.className = "success-message";
                  successMessage.style.backgroundColor = "#4CAF5020";
          ...
          // Add the event listener for OTP submission
